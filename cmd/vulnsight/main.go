...
on extensive configurability, massive extensibility and ease of use.`)
	...
	*/
	...
	flagSet.CreateGroup("templates", "Templates",
		flagSet.BoolVarP(&options.NewTemplates, "new-templates", "nt", false, "run only new templates added in latest vulnsight-templates release"),
		...
		flagSet.BoolVar(&options.SignTemplates, "sign", false, "signs the templates with the private key defined in VULNSIGHT_SIGNATURE_PRIVATE_KEY env variable"),
	...
	flagSet.CreateGroup("debug", "Debug",
		...
		flagSet.CallbackVarP(printTemplateVersion, "templates-version", "tv", "shows the version of the installed vulnsight-templates"),
	...
	flagSet.CreateGroup("update", "Update",
		...
		flagSet.BoolVarP(&options.UpdateTemplates, "update-templates", "ut", false, "update vulnsight-templates to latest released version"),
		flagSet.StringVarP(&options.NewTemplatesDirectory, "update-template-dir", "ud", "", "custom directory to install / update vulnsight-templates"),
		flagSet.CallbackVarP(disableUpdatesCallback, "disable-update-check", "duc", "disable automatic vulnsight/templates update check"),
	)
...
Run vulnsight with sorted Markdown outputs (with environment variables):
	$ MARKDOWN_EXPORT_SORT_MODE=template vulnsight -target example.com -markdown-export vulnsight_report/

Additional documentation is available at: https://github.com/laiyuci/vulnsight
	`)
...
func printTemplateVersion() {
	...
	options.Logger.Info().Msgf("Public vulnsight-templates version: %s (%s)\n", cfg.TemplateVersion, cfg.TemplatesDirectory)
...
func processInlineSecretsFromProfile(profilePath string, options *types.Options) (string, error) {
	...
	tempDir := filepath.Join(os.TempDir(), "vulnsight-secrets")
